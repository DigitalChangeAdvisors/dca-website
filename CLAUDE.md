...
### Persistencia — conectada y verificada (2026-09-13)
`SUBMIT_ENDPOINT` apunta a un Google Apps Script Web App ("Umbral Estudio Fuga - Backend") vinculado a la Google Sheet **"Umbral Estudio Fuga - Respuestas"** (Drive de `ceo@digitalchangeadvisors.com`, ID `1jOU_ztohyMcqvsp72qucSc5tr62QveFaG9pE_v-EyIQ`). Mismo stack ya usado en "AI Return Pulse" y la Bitácora de Hoja de Ruta de este proyecto. El mecanismo: `fetch(SUBMIT_ENDPOINT, {method:'POST', mode:'no-cors', body: JSON.stringify(payload)})`, con dos acciones distintas (`action:'submit'` al terminar el formulario, `action:'click'` si hace clic en el enlace de salida), correlacionadas por un `submissionId` generado en el cliente.
- **Despliegue realizado con `clasp`** (ya instalado y usado en este proyecto para "AI Return Pulse") — creado con `clasp create-script --parentId <id de la Sheet>` (vinculado, no standalone), manifiesto `appsscript.json` con `"webapp": {"executeAs": "USER_DEPLOYING", "access": "ANYONE_ANONYMOUS"}`, `clasp push` + `clasp create-deployment`. El código fuente del script vive fuera de este repo (en el propio proyecto de Apps Script, editable desde `script.google.com`) — mismo criterio que "AI Return Pulse", no se versiona aquí.
- **Verificado en vivo (2026-09-13):** `action:'submit'` y `action:'click'` probados end-to-end contra el endpoint real, confirmando fila escrita en la Sheet con todos los campos y `ClickedArt`/`ClickedAt` correlacionados correctamente por `submissionId`. Quedaron 2 filas de prueba en la hoja (`SubmissionId` vacío y `qa-test-verify-001`) — borrarlas antes de lanzar la campaña.
- **Hallazgo de diagnóstico (no repetir):** una prueba inicial con `curl -L` (que sigue redirecciones automáticamente reenviando el método/cuerpo original) dio un falso positivo de "bloqueado por política de Workspace" — la página de error de Drive apareció porque `curl -L` reenviaba la petición al segundo salto (`script.googleusercontent.com/macros/echo`) de forma distinta a como lo hace un navegador real. El endpoint funciona correctamente cuando se seguía el redirect como lo haría un `fetch()` de navegador (sin forzar el método/cuerpo original en el segundo salto) — verificado haciendo el salto manualmente. Ninguna configuración de Workspace (uso compartido de Drive, Control de acceso a apps) estaba bloqueando nada; se auditaron y confirmaron sin restricciones antes de encontrar la causa real.

### Enlace de salida — excepción deliberada de enrutamiento (2026-09-13)
El único botón de U-02 abre **directamente el formulario Tally del AI Return Test** (`https://tally.so/r/Np6e5W`, no `/art/`) en pestaña nueva, con UTM propios (`utm_source=umbral&utm_medium=devolucion&utm_campaign=estudio-fuga-2026`) más el `origen` capturado al entrar a `/umbral` — no pisa el UTM con el que llegó el visitante a `/estudio-fuga`, lo encadena.

**Por qué se salta `/art/` — esto rompe deliberadamente la regla "Tally directo solo desde los CTAs internos de `/art`"** (ver "Enrutamiento de CTAs y Enlaces" más abajo): el visitante que llega a este botón ya respondió 5 preguntas, recibió una lectura personalizada y decidió activamente continuar — es un visitante en etapa de Cierre, no uno frío que necesite que `/art/` lo persuada de qué es el AI Return Test (ya se lo explicó la propia devolución). Insertar la landing de venta ahí repetiría un argumento ya procesado en la misma sesión — mismo principio ya vigente para "no repetir contenido ya visto" (ver Behavioral Economics, CLAUDE.md raíz) y el mismo criterio que ya llevó a que los CTAs de header/hero de las landings ARA vayan directo al instrumento, no a un ancla intermedia.
- **Lo que se pierde al saltar `/art/`:** su nota de confianza. Compensado agregando `<p class="result-trust">Sin compromiso · Tu reporte es confidencial</p>` bajo el botón — versión recortada de la nota canónica del CTA ("Sin compromiso · 55 preguntas, menos de 30 minutos · Tu reporte es confidencial"; se omite el tramo de preguntas/minutos porque el párrafo justo arriba ya lo dice, evitar repetirlo en la misma pantalla).
- Este es el **único punto del sitio, fuera de `/art/`, que enlaza directo a Tally** — no replicar este patrón en otra página sin la misma justificación (visitante ya cualificado por un flujo propio, no tráfico frío).

## Escalera de Valor de `/returnai` — 2 rutas nuevas, conteo corregido (2026-09-08)

> Petición directa del usuario, copy 100% suministrado (no generado). Marcado hecho a mano en `returnai.html`, sección `id="sistema-returnai"` — 8 filas `.vl-row` en total repartidas en 2 grupos (`Nivel de Entrada`: 5, `Nivel de Resultados Específicos`: 3); el "Nivel de Resultados Escalables" es un componente aparte (`.vl-flagship-row`, una sola oferta "Solución ReturnAI") y no cuenta como fila de la escalera.

- **H2 de la sección** cambiado de "Siete rutas de captura de valor. Un solo destino: el retorno verificable." a "Rutas de captura de valor con un solo destino: el retorno verificable." — ya no lleva el número, evita que quede desactualizado si la escalera vuelve a crecer.
- **Tarjeta "Novela ReturnAI"** (Nivel de Entrada, 2ª posición): **ya existía** antes de este cambio con otro copy — el usuario pidió originalmente "tarjeta nueva" sin saber que ya estaba ahí. Confirmado con el usuario: se reemplazó el contenido de la fila existente, no se duplicó. CTA reducido de 2 enlaces (que pedía el usuario: "Comprar en Amazon" + "Conocer la novela") a 1 solo — el componente `.vl-row__cta` de toda la escalera soporta un único `<a>`; el usuario confirmó no introducir una variante nueva solo para esta fila y quedarse con el CTA primario (Amazon). El enlace "Conocer la novela" no se perdió — `/novela-returnai/` ya está enlazado desde el nav y el homepage.
- **Tarjeta "Auditoría de Fuga de Productividad de la IA"** (nueva, Nivel de Resultados Específicos, 1ª posición, antes de AI Return Unlock): única fila genuinamente nueva de este cambio.
- **Conteo de rutas corregido de 7 a 8, no a 9.** El usuario pidió "9" asumiendo que ambas tarjetas (Novela + Auditoría) eran adiciones nuevas — pero la Novela ya contaba en el total de 7. El conteo real tras el cambio es 7 + 1 = 8. Corregido en los 3 lugares que el usuario señaló (`data-count` del contador animado del hero, párrafo del hero, párrafo introductorio de la sección) — verificado contando literalmente los `<div class="vl-row">` del marcado antes de escribir el número, no asumido.
- **Contador no derivado dinámicamente del DOM — reportado y dejado codificado, con autorización explícita del usuario.** `app.js`/`countUp()` solo anima un `data-count` ya escrito en el HTML, no cuenta elementos; y las 2 menciones en prosa ("ocho rutas", "ocho formas") son texto en español que no se puede derivar sin un conversor número→palabra. Habría sido el único contador de todo el sitio con ese mecanismo — se mantiene el patrón establecido (cifra escrita a mano, verificada explícitamente en cada cambio).
- Verificado tras el cambio: 0 apariciones de "25 minutos"/"25′" en la página (ya corregido en una sesión anterior, sección 2.6), 0 apariciones de "siete"/"Siete" como contador de rutas (solo queda "cuarenta y siete minutos" dentro del extracto de la novela, sin relación), HTML válido, anclas de navegación (`id="sistema-returnai"`) sin tocar.
...
### `/autor/cesar-lozano` — 8 bloques, sin plantilla previa en el sitio
Página nueva, autocontenida: carga solo `styles.css/4/5/6` (sin CSS de otra página) más un `<style>` propio al final del archivo con las clases `au-*` (hero, stats, narrow, source, books, case, cta-note). Reutiliza `app.js`/`enhance4.js`/`enhance6.js` para el sistema de reveal-on-scroll estándar del sitio. Foto: `assets/img-leader-1.{jpg,webp}` (la misma que ya usa `/nosotros`, sin generar un asset nuevo). Los 8 bloques siguen el orden exacto de la sección 3.2 de la especificación: declaración de posición → credencial → problema (con cita McKinsey completa) → autoría verificable (tabla de 8 libros, edición propia declarada donde corresponde) → caso propio (producción del libro, 18 meses → 16 semanas, 4,5x) → instrumento (AI Return Test) → lo que no hacemos (filtro de interlocutor) → cómo se trabaja + CTA único a la medición.
- **Bug encontrado y corregido en el propio build:** los dos botones `.btn--gold` de la página (bloques 6 y 8) se renderizaban como enlaces azules subrayados sin estilo — `.btn--gold` está definido en `styles-modelo-aria.css`/`styles-returnai7.css`, ninguna de las dos cargadas por esta página nueva. Corregido declarando `.btn--gold`/`.btn--gold:hover` en el `<style>` propio de la página, con los mismos valores que `styles-modelo-aria.css` (fondo `var(--gold)`, texto blanco). Detectado con captura de pantalla real vía Chrome headless + CDP antes de dar por terminada la página — no se verificó solo leyendo el HTML.
- **Nota de accesibilidad heredada, no introducida aquí:** `.btn--gold` con texto blanco da un contraste de 3.67:1 sobre `--gold` (`#a48111`) — por debajo del mínimo WCAG AA de 4.5:1 para texto normal (el tamaño del botón, 15-16px semibold, no alcanza el umbral de "texto grande" que permitiría 3:1). Es el mismo patrón ya usado en `modelo-aria.html`, `returnai.html` y los 17 artículos de Perspectivas — no se corrigió aquí para no introducir una inconsistencia visual entre esta página nueva y el resto del sitio. Queda como hallazgo pendiente de una futura pasada de hardening de accesibilidad a nivel de sitio, no específico de esta página.
- `Person` JSON-LD: `name`, `jobTitle`, `description` (declaración de posición literal), `nationality`, `url`, `worksFor` (enlazado por `@id` a la `Organization` del homepage), `knowsAbout`, `sameAs` (LinkedIn ya existente, `linkedin.com/in/ingcesarlozano`), `author[]` con los 8 libros como `Book` + `isbn` donde existe.
- El `Organization` JSON-LD del homepage y el `Person` JSON-LD independiente de `/nosotros` se actualizaron para agregar `url: ".../autor/cesar-lozano"` al founder/Person de César — sin tocar el de Ruth (fuera de alcance, Fase 2 del sistema de autoridad).

### Nav — 5º ítem y ajuste de breakpoint (ver también CLAUDE.md raíz)
"El autor del Modelo ARIA" → `/autor/cesar-lozano` se insertó entre "ReturnAI" y "Perspectivas" en los 26 archivos que llevan la nav completa (`nav__links` + `mobile-menu`): homepage, `/modelo-aria`, `/returnai`, `/blog`, `/nosotros`, `/contacto`, `/arquetipos`, `privacy-policy`, `terms-of-use` y los 17 artículos de Perspectivas — vía script Python con verificación de exactamente 1 coincidencia por archivo antes de escribir, no reemplazo manual. Las 7 landings ABM, `/art`, `/novela-returnai`, `/ara` y `centro-baip` no llevan nav completa (son páginas de embudo autocontenidas) — no se tocaron.
- **Regresión visual detectada y corregida antes de dar por cerrado el cambio:** con 5 ítems, la nav desbordaba horizontalmente entre 881px y ~1240px de viewport — verificado con capturas reales vía Chrome headless (CDP), no solo inspección de CSS. Fix de dos partes: (1) `.nav__links { gap: 34px→24px }` + `white-space:nowrap` en los links y el botón CTA, para evitar que el texto se partiera en 2-3 líneas dentro de cada ítem; (2) nueva regla `@media (max-width: 1259px)` que colapsa `.nav__links`/`.nav__cta .btn` a favor del menú hamburguesa — separada del breakpoint general de 880px que usan el resto de los componentes de página (hero, stats, rungs), para no tocar ningún otro elemento responsive del sitio. Verificado sin overflow en 1024/1150/1260/1280/1440px y menú hamburguesa funcional en mobile (390px) con el nuevo ítem incluido.

### Home — saneamiento del bloque de evidencia ilustrativo (sección 4 de la especificación)
El bloque "Caso Ancla" del homepage (`index.html`, antes de la sección Perspectivas) presentaba una cifra de inversión, uso antes/después y un testimonio entrecomillado atribuido a un "Director de Transformación Digital de una gestora de inversiones" **sin identidad verificable** — el usuario confirmó en la especificación que es una composición ilustrativa y autorizó su retiro completo.
- **Retirado:** texto, cifras (`$3.2M`, `19%→67%`, `+13 pts`, `120 días`), testimonio y atribución — íntegros, sin dejar remanente.
- **Sustituido, misma posición en el recorrido** (después de la declaración de categoría, antes del CTA), por una sección "Validación de campo del diagnóstico" con 3 tarjetas — los 3 testimonios REALES y autorizados de la sección 2.8 de la especificación (Rubén Darío Cañas/Subocol, Julián Hurtado/Susuerte, Diego Parra/SUMMA), ya publicados en `/art/`. Cada tarjeta usa una sola línea del texto autorizado, extraída verbatim (sin parafrasear) — foto, nombre, cargo y una frase de la cita real.
- **Rótulo de sección: "Validación de campo del diagnóstico"** — nunca "evidencia de resultados" ni "casos de éxito". Enlaza a la sección completa en `/art/#validacion` (id nuevo agregado a esa sección para el ancla).
- Nota al pie conservada verbatim: "Los testimoniales corresponden a líderes que aplicaron el diagnóstico en sus organizaciones durante la fase de validación del instrumento. Sus nombres, cargos y empresas son verificables públicamente."
- CSS nuevo, scoped: `.validation-grid`/`.validation-card`/`.validation-footnote` en `styles.css`, junto al bloque `.special`/`.case__` ya existente. Opacidad del footnote (`rgba(255,255,255,0.5)` sobre `--carbon`) verificada en 4.78:1 — cumple AA.
- El CTA de esta sección cambió de "¿Cuánto retorno está dejando sobre la mesa tu inversión en IA?" (framing de pérdida, redundante con el diagnóstico frío del hero) a "¿Dónde se está perdiendo el retorno de tu inversión en IA?" — mismo mecanismo, evita repetir verbatim un framing que ya no tiene el caso ancla ilustrativo detrás para sostenerlo.

### Par canónico del AI Return Test — "55 preguntas · menos de 30 minutos" reemplaza "25 minutos" en todo el sitio (sección 2.6 de la especificación)
El libro impreso de ReturnAI declara 55 preguntas y menos de 30 minutos — cifra inmutable. La web declaraba "25 minutos" en `/art/` (7 instancias: meta description, og:description, hero, quote, CTA note, stat visual, H2 de cierre) sin mencionar el número de preguntas. **Alcance ampliado a todo el sitio tras confirmación explícita del usuario** (la sección 9 de la especificación solo nombraba `/art/` como entregable, pero la sección 2.6 decía "reemplazar toda aparición... la web cede" — se resolvió la ambigüedad preguntando antes de tocar los ~20 archivos adicionales).
- **Corregido en 26 archivos:** `/art/` (7), `/returnai` (2, incluida la fila del Test en la Escalera de Valor), `/modelo-aria`, `/nosotros`, `/contacto`, `/blog`, homepage (3), `arquetipos/`, `novela-returnai/` (3) y los 17 artículos de Perspectivas (`article-paper00`–`16`, 2 instancias cada uno salvo `article-paper07` con una tercera en el cuerpo del texto).
- **Deliberadamente NO corregido:** `Homepage v6.html` y `modelo-aria-v2.html`…`v6.html` — snapshots históricos ya excluidos del deploy por la Regla 6 del CLAUDE.md raíz ("no modificar, no deployar"). Tocarlos habría sido esfuerzo desperdiciado sobre contenido que nunca se publica.
...
### Incidente — `/autor`, `/autor/cesar-lozano`, `/autor/ruth-jaramillo` sin barra final (encontrado y corregido 2026-09-09)

Auditoría de regresión rutinaria (`scripts/verify-robots.sh`) encontró que las 3 páginas de autor creadas en esta Fase 3 se dieron de alta **sin la barra final**, contradiciendo la propia "Regla derivada" ya documentada arriba en este archivo para `/arquetipos` (verificar con `curl -sIL` cuál forma resuelve `200` directo antes de fijar cualquier canonical/sitemap/enlace interno). Verificado en vivo: `/autor/cesar-lozano` → `301` → `/autor/cesar-lozano/` → `200` (mismo patrón que `/ara/` y `/arquetipos/`, GitHub Pages sirviendo `index.html` de subdirectorio).

- **Alcance real:** el `<link rel="canonical">`, `og:url` y `"url"` del JSON-LD de las 3 páginas propias, más el `<loc>` de las 3 en `sitemap.xml`, **y el enlace de nav "El autor del Modelo ARIA" en las 29 páginas que lo llevan** (homepage, `/modelo-aria`, `/returnai`, `/nosotros`, `/contacto`, `/blog`, `/arquetipos`, `privacy-policy`, `terms-of-use`, los 17 artículos y las propias 3 páginas de autor) — cero excepciones, el 100% de las referencias al slug `/autor...` en todo `website/` usaban la forma sin barra. Cada clic en ese ítem de nav disparaba un salto 301 innecesario.
- **Fix:** barrido estructural con Python (`str.replace` de las 3 formas exactas entre comillas, sobre todo `website/**/*.html`) — no lista manual, mismo criterio que las Reglas 5/6 del `CLAUDE.md` raíz. `sitemap.xml` y `lastmod` de las páginas tocadas actualizados en el mismo barrido.
- **Estado:** corregido en el árbol de trabajo, **no desplegado todavía** — `scripts/verify-robots.sh` sigue marcando el fallo en vivo (curl contra producción) hasta el próximo `./deploy.sh website`. No es una regresión nueva del `./deploy.sh` — el canonical real en producción no cambia hasta publicar.
...
### `article-paper11.html` — pieza 1 de la validación académica prometida en paper10 (2026-09-02)
Primer paper construido con el flujo `/perspectiva` completo desde un `.md` fuente (`Perspectivas-2026-Pieza-1-NBER-Existencia.md`). Fuente: NBER Working Paper 34836 (*Firm Data on AI*), encuesta de ~6.000 ejecutivos en 4 países vía Fed de Atlanta/Banco de Inglaterra/Bundesbank/Universidad Macquarie — 0,29% de productividad realizada en 3 años vs. 1,44% esperado en los tres años siguientes. Autor César Lozano, badge "Investigación", `data-stage="evidencia"` (confirmados con el usuario antes de construir, per Paso 2 del flujo).
- **Es la continuación directa de `article-paper10.html`:** ese paper cerraba con una lista de 6 piezas futuras, y la primera —"si el fenómeno resiste una medición académica independiente de la industria de la consultoría"— es exactamente este artículo. Se actualizó ese `<li>` en `article-paper10.html` para que enlace hacia `article-paper11` en vez de quedar como texto plano sin ruta — la promesa ya tiene destino.
- **Citación doble en JSON-LD:** además de la cita primaria del NBER (con los 13 autores nombrados, per la regla de verificabilidad del CLAUDE.md raíz), se reutiliza verbatim la cita de McKinsey *The State of AI in 2026* ya usada en `article-paper10.html`, porque este artículo repite la cifra de "43 puntos porcentuales" (lectura propia de DCA, no dato de McKinsey) — misma cifra, misma cita, sin reformular la fuente dos veces.
- **`.art-title--long` estrenada aquí** — ver nota de abajo.
- **Pendiente de tu parte antes de desplegar:** generar la imagen con el prompt de Nano Banana 2 entregado en la conversación y devolverla — `assets/blog-article-paper11.{webp,jpg}` todavía no existen como archivos reales, el HTML ya referencia esos nombres. Sin la imagen, no se corre `./deploy.sh website` (el hero y la tarjeta del catálogo quedarían rotos en producción).

### `article-paper17.html` — Colombia AI Pulse, primer artículo construido tras el Sistema de Autoridad Personal (2026-09-11)
Fuente: `Perspectivas-2026-Indice-de-madurez-sin-moneda-Colombia-AI-Pulse.md` — a diferencia de las piezas 1 y 2 de la serie, este documento llegó como "hoja de publicación" ya formateada para CMS (título/descripción/badge/autora/puntos clave pre-decididos por el usuario) y con su propio bloque de JSON-LD sugerido. Autora Ruth Jaramillo (dada en el propio documento, no se preguntó). Badge "Investigación · Tesis DCA" + `art-source-note` "Fuente analizada: Endeavor Data Unit y Xertica.ai, 2026" — el documento ya llegaba con la separación etiqueta/fuente del Protocolo de Reencuadre Autoral aplicada, así que no hizo falta ninguna de las 7 intervenciones de ese protocolo: se construyó directo en su forma final.
- **Dos decisiones que se apartaron de lo que pedía el documento fuente, ambas explicadas al usuario en el momento:**
  1. **URL:** el documento sugería `/perspectivas/indice-madurez-ia-colombia-fuga-productividad` y un JSON-LD con nodos `Article`/`Person`/`Report` enlazados por `@id` — un esquema de CMS distinto al que existe hoy en el sitio. Se publicó como `article-paper17.html` (continuando la numeración secuencial de las 17 piezas ya existentes) y con el `@graph` real del sitio (`BlogPosting` + `citation[]` + `DefinedTerm`, patrón de `article-paper16.html`) — cambiar de esquema de URL/datos estructurados habría exigido infraestructura nueva (ruta `/perspectivas/`, nav, cruces) no solicitada explícitamente para esta pieza.
  2. **Bloque nuevo, sin precedente en los 17 artículos anteriores:** el documento invita a un "Estudio de Fuga de Productividad de la IA · Colombia 2026" (cupo 40 empresas, cierre de campo 16 oct 2026) con un enlace de registro marcado como pendiente en el propio documento (`[enlace al registro del estudio, con UTM de la pieza]`). Consultado con el usuario: se incluyó el texto del estudio, con el CTA ("Medir mi organización →") enlazando de forma interina al AI Return Test (`/art/?utm_source=article-p17...`) — el propio texto dice que cada participante completa ese instrumento. **Validado en la auditoría de regresión del mismo día (2026-09-11):** el pull posterior trajo la sección `.field-study` de `blog.html` (construida en paralelo, commits `e63e7bd`/`e8207fa`/`05ecacc`), con su propio CTA "Medir mi empresa" apuntando exactamente al mismo destino (`/art/`) — confirma que no existe ni existirá una página de registro dedicada; el enlace interino era, de hecho, el destino definitivo. Cifras cruzadas y consistentes entre ambos (40 empresas, 55 preguntas/30 min, 16 oct, 3 nov, publicación enero 2027).
- **Citación:** `Colombia AI Pulse: Adoptar ya no basta` (Endeavor Data Unit + Xertica.ai, Data Unit Report #007, agosto 2026) — autores declarados como `Organization` (no `Person`), sin campo `publisher` separado porque el documento fuente no distingue uno de los otros. Método/muestra declarados en la propia `description` de la citación (493 profesionales, muestra no probabilística) per la regla de verificabilidad.
- **`DefinedTerm` único** — "Fuga de productividad de la IA", texto reutilizado verbatim de `article-paper16.html` (no se introduce ningún componente nuevo del Modelo ARIA en esta pieza).
- Related cards eligen los otros dos artículos "evidencia" de la serie con estudios externos regionales/globales de adopción (`article-paper12`, CEPAL/Brasil — la pieza hermana más cercana en forma y tesis) y `article-paper11` (NBER) + `article-paper10` (McKinsey, apertura de la serie).
- **Imagen:** validada en dos rondas contra los 3 recortes reales — la primera versión incluía un prop con texto legible fabricado ("AI Maturity Index Scorecard", "Score: 68%") que contradecía las cifras reales citadas en el artículo (45,6/100) y violaba el brief ("no text, no graphic overlays"); rechazada. La segunda versión resolvió el prop a texto de párrafo genérico ilegible — aprobada y montada.
- **Regresión encontrada y corregida antes de commitear:** el artículo se construyó *antes* de hacer `git pull` (el monorepo local estaba 9 commits detrás). Reutilizaba por eso 3 patrones ya corregidos en el resto del sitio por commits paralelos del mismo día: `/autor/cesar-lozano` y `/autor/ruth-jaramillo` sin barra final (2 en nav/mobile-menu + 1 en el enlace del módulo de autor + 1 en `author.url` del JSON-LD, que además faltaba por completo), y el footer "Agendar sesión" apuntando directo a `tidycal.com/pwgdigital/returnai` en vez del dominio de marca blanca `agenda.digitalchangeadvisors.com/retorno-ia`. Corregido con sustitución literal verificada (conteo exacto de 1 coincidencia por patrón) antes del commit — ver Regla 14 más abajo, que generaliza esta lección.

### Hueco identificado — parcialmente resuelto (2026-09-02)
`article-paper09.html` (y `article-paper03.html`) siguen resolviendo el `<h1 class="art-title">` de 3 líneas con `style="font-size:clamp(...)"` **inline** — no se tocaron, fuera de alcance de la sesión que los encontró. La clase `.art-title--long` (`font-size: clamp(22px,2.4vw,32px); max-width: 40ch`) ya existe en `styles-article.css` desde `article-paper11.html`, que es el primer paper en usarla en vez del inline style. **Regla para el próximo titular de 3 líneas: usar `.art-title--long`, ya no hace falta crearla.** Retrofit de paper03/paper09 queda pendiente, no urgente — son cosméticamente idénticos a la clase, solo repiten el valor a mano.

## Colecciones por Semestre en `/blog` — reorganización del catálogo (canónico 2026-09-04)

> El catálogo (`#catalogo`, 17 tarjetas) pasó de ser una lista plana ordenada por fecha a dos bloques agrupados por semestre editorial, cada uno con nombre de colección temático — pedido explícito del usuario. Gate BE+UI-UX corrido antes de implementar: agrupar por colección con semestre visible es un mecanismo de autoridad institucional correcto para la etapa Validación de este visitante (ya viene de nav buscando contenido, no diagnóstico frío); sin señales de alarma.

### Estructura (más reciente primero)
- **Semestre II · 2026 — "Fuga de Productividad de la IA"** (8 piezas: `article-paper10`–`16` + `arquetipos/`). Nombre dado por el usuario — coincide con la tesis que corre por 7 de las 8 piezas.
- **Semestre I · 2026 — "El Mapa de la Brecha IA"** (9 piezas: `article-paper01`–`09`). Nombre elegido por el usuario entre 3 opciones argumentadas (contraste narrativo con "Fuga": primero se traza el mapa completo del territorio — gobernanza, adopción, ROI, liderazgo —, después se ubica un mecanismo específico dentro de ese mapa).
- Orden interno de cada bloque: estrictamente descendente por fecha de publicación — `arquetipos/` ya no está fijo en primera posición, se intercala donde le corresponde por su propia fecha (5 ago 2026).
- Alejandro Ríos ("Your AI isn't failing") no se tocó — sigue destacado en el masthead, fuera del grid; el usuario confirmó que ahí debía quedarse.

### Redistribución de fechas de Semestre II
Las 7 piezas de la serie (antes apiñadas 1–4 sep 2026) se redistribuyeron entre julio, agosto y septiembre para que el catálogo no se lea como una ráfaga de un fin de semana: paper10→8 jul, paper11→22 jul, paper12→12 ago, paper13→19 ago, paper14→26 ago, paper15→2 sep. `arquetipos/` (5 ago) y paper16 (4 sep, fecha real de publicación) quedaron sin cambio. Actualizado en 3 lugares por artículo (meta `article:published_time`, JSON-LD `datePublished`, byline visible) más `blog.html` (tarjeta + `Blog.blogPost[]`) y `sitemap.xml` (`lastmod` real de la fecha de edición, 2026-09-04 — no confundir con la fecha editorial ficticia del `datePublished`, son conceptos distintos).

### Franja "Fundamentales" — reemplaza "Lo último" (no se agrega como capa adicional)
El usuario propuso originalmente una fila de 3 artículos "anclados" adicional antes de las secciones. Se le señaló el riesgo: sumada al masthead (1) y a la franja de recencia (3), el tope de página habría tenido 7 tarjetas "especiales" antes del catálogo organizado — sobrecarga que diluye qué es realmente destacado. Se acordó **reemplazar** la franja de recencia ("Lo último": paper16/15/14) por una franja de profundidad ("Fundamentales"), no apilar una tercera capa.
- **Selección — 3 piezas, ninguna duplicada con el masthead:**
  1. `article-paper10` (César Lozano) — tesis de apertura de toda la Serie 2, punto de entrada natural a la colección más grande del catálogo.
  2. `arquetipos/` (Ruth Jaramillo) — el único contenido verdaderamente atemporal del catálogo, marco de diagnóstico propietario no atado a un estudio externo de un momento específico.
  3. `article-paper02` (César Lozano) — introduce el primer componente del Modelo ARIA (AI North Star), representa al Semestre I ("El Mapa de la Brecha IA").
- **Autoría de `arquetipos/` cambiada de César Lozano a Ruth Jaramillo** (decisión explícita del usuario) para que la fila tuviera presencia de Ruth sin desplazar a ninguno de los otros dos. La página `arquetipos/index.html` no lleva firma de autor propia — la atribución vive solo en las tarjetas del catálogo — así que el cambio no contradice nada en la página fuente. Aplicado de forma consistente en las 2 tarjetas donde aparece esta pieza (grid de Semestre II y franja "Fundamentales"). Sustituye la nota de atribución a César del 2026-08-05 (ver sección `/arquetipos` abajo, actualizada).
- Reutiliza el markup `.masthead__rail`/`.rail-item` existente sin cambios de CSS — solo cambia el texto de `.rail__head` y los 3 `<a class="rail-item">`.

### Filtro por tema — fix de vacío por bloque
El filtro por tema (chips `Todas`/`Comportamiento`/`Métricas`/`Gobierno`/`Evidencia`) sigue aplicando a las 17 tarjetas sin importar el bloque. Se agregó lógica en el script de filtro: si un bloque de semestre queda en 0 tarjetas visibles tras filtrar, el bloque completo (encabezado + grid) se oculta con `display:none` — evita un encabezado de colección quedando visible sobre un grid vacío.

### CSS — cero clases nuevas de familia distinta
`.article-grid-wrap .section-head h3 { margin-bottom: 20px }` (la base solo definía ese margen para `h2` dentro de `.section-head`) y `.article-grid-wrap .collection-block + .rule { margin: 64px 0 }` (separador entre bloques) — ambas reutilizan clases (`.section-head`, `.label`, `.rule`) ya usadas en todo el sitio.

## Protocolo de Reencuadre Autoral — completado en las 16 piezas (2026-09-08)

> Documento rector: `website/PROTOCOLO_Reencuadre_Autoral_Perspectivas_DCA.md` (comiteado, íntegro). Objetivo: que un motor generativo lea primero el pensamiento propio de DCA (etiqueta, metadatos, definición propietaria, puntos clave) y solo después la firma externa citada — sin reescribir el cuerpo argumental ni debilitar ninguna atribución externa. Ejecutado en Fase 1 (auditoría) + Fase 2 (piloto en `article-paper16`, aprobado) + Fase 3 (4 tandas de 4, todas aprobadas y desplegadas) + verificación cruzada final. `article-paper00` excluido por completo, tal como manda el protocolo — con una única excepción puntual autorizada después (ver más abajo).

### Las 7 intervenciones aplicadas, patrón consistente en las 16 piezas
1. **Etiqueta** — en los 8 artículos cuyo badge mezclaba taxonomía + firma (`Gobernanza · Bain`, `Investigación · MIT`, etc.), se separó en `<taxonomía> · Tesis DCA` + `<span class="art-source-note">Fuente analizada: <firma>, <año></span>` (clase nueva, `styles-article.css`, jerarquía visual menor que `.art-cat`, nunca oro). Los 8 badges que ya no mezclaban firma (`Análisis · Evidencia`, `Marco · Liderazgo`, etc.) se dejaron intactos — la intervención no aplica si no hay nada que separar.
2. **Metadatos invertidos** — `<title>`/`meta description`/`og:title`/`og:description`: afirmación propia primero, fuente después. Se dejaron sin tocar los campos que ya venían DCA-first (varios ya lo estaban antes del protocolo).
3. **Definición propietaria elevada** — donde ya vivía cerca del inicio (patrón de plantilla 2026: paper10–16), sin cambio. Donde vivía en el cierre (plantilla anterior: paper01, 03, 04, 05, 06), se compuso un bloque nuevo cerca del inicio con el texto literal — **el párrafo original de cierre se dejó intacto**, por decisión deliberada: moverlo de verdad habría roto referencias pronominales o el remate del artículo; la distancia entre el bloque nuevo y el original evita que la repetición se lea como error de redacción (más abajo, la flexibilización que lo autorizó).
4. **Puntos clave reordenados** — afirmación propia primero, cifra externa después, en los artículos donde el orden original no ya lo hacía.
5. **Bloque "La lectura de Digital Change Advisors"** — nuevo `.art-callout` en los 16, compuesto solo con texto ya existente en cada artículo (nunca contenido inventado).
6. **Un encabezado por artículo convertido a pregunta** — máximo 2 por el protocolo, se usó 1 en los 16. Excepción de mayor alcance: en `arquetipos/index.html` (Tipo B) se convirtió también el **H1 y el `<title>`** a pregunta ("¿Qué arquetipo organizacional bloquea el retorno de tu inversión en IA?") y en `article-paper04` (tratado como Tipo B por decisión explícita del usuario) igual — ambos casos con autorización explícita previa, porque cambiar el H1/`<title>` de una página ya indexada excede el alcance normal de "encabezado interno como pregunta" que aplica al resto.
7. **Datos estructurados** — nuevo `DefinedTerm` por artículo (17 en total, incluidos los 3 de `article-paper16` del piloto) declarando el término propietario central de cada pieza, todos con `publisher.@id` apuntando a `https://digitalchangeadvisors.com/#organization`. JSON-LD reestructurado a `@graph` para poder declarar `BlogPosting` + `DefinedTerm` en el mismo bloque sin romper el `citation[]` existente. Enlaces salientes duplicados a la misma fuente (bajada + cuerpo) reducidos a uno solo, en la sección donde se expone el dato.

### Flexibilización del protocolo autorizada en sesión (regla derivada, aplica a futuras piezas)
El usuario autorizó explícitamente: *"cuando por efectos del reacomodo queden textos aparentemente repetitivos, puedes intervenir el cuerpo del artículo para ajustar el componente que menos impacto tenga en el posicionamiento GEO y en el objetivo de resaltar el pensamiento DCA, manteniendo la coherencia del mensaje y la continuidad orgánica"*. Se usó una sola vez, en el piloto (`article-paper16`): se recortó un párrafo de intro que quedó duplicando casi palabra por palabra el nuevo bloque de lectura propia, adyacente a él. En el resto de los 15 artículos no hizo falta invocarla — la redundancia quedó a suficiente distancia (bloque nuevo cerca del inicio vs. párrafo de cierre original) para no leerse como error.

### Hallazgos corregidos en la verificación cruzada final
- `llms.txt` no se sincroniza automáticamente con cambios de `<title>`/H1 — quedó citando los títulos antiguos de `article-paper04` y `article-paper06` tras el reencuadre (corregido), y nunca había incluido `article-paper10`–`16` desde que se publicaron (gap preexistente, corregido en la misma pasada — los 17 papers ya están referenciados).
- Meta descriptions de `paper02`/`04`/`05`/`08` (163–167 caracteres, fuera del rango 120–155 del proyecto) — deuda preexistente a esta sesión, no introducida por el reencuadre; acortadas sin alterar su framing DCA-first ya vigente.
...
- **Tarjeta en el catálogo de `/blog`** (`#catalogo`): reutiliza el componente `.article-card` existente sin CSS nuevo, badge `Marco` (patrón ya usado por otro contenido de framework en el catálogo), filtro `data-stage="adopcion"` (Comportamiento & uso). Título y excerpt reutilizan el H1 y la meta description ya aprobados de la página — sin copy nuevo.
  - **Atribución — actualizada 2026-09-04:** la tarjeta pasó de César Lozano a **Ruth Jaramillo** (decisión explícita del usuario, al reorganizar `/blog` en la fila "Fundamentales" — ver sección "Colecciones por semestre" abajo). Motivo: dar presencia de Ruth en esa fila de 3 sin desplazar a ninguno de los otros dos artículos elegidos (ambos de César). La página `arquetipos/index.html` en sí no lleva firma de autor propia (es un framework atribuido a la firma, no a una persona) — la atribución vive únicamente en las tarjetas del catálogo, así que el cambio no contradice nada en la página fuente. Aplicado de forma consistente en las 2 tarjetas donde aparece (grid de Semestre II y fila "Fundamentales"). La nota anterior (atribución a César, 2026-08-05) queda retirada.
...
- **Agendamiento — DOS enlaces canónicos distintos, no intercambiables (corregido 2026-09-09):**
  ...
  - **Por qué (incidente 2026-09-09):** `/returnai` y otras 21 páginas seguían enlazando directo a `tidycal.com/pwgdigital/returnai` en vez del dominio de marca blanca — reportado como enlace roto por el usuario. Al corregirlo se encontró un segundo error: las 7 landings ABM usaban `agenda.digitalchangeadvisors.com/returnai` (slug sin `-art`, no es un endpoint válido para esa página) en vez de `/returnai-art`. Una ronda de auditoría posterior encontró además 2 CTAs mal clasificados en la primera pasada (`/blog` y `/returnai`, copy "ya tienes tus resultados/diagnóstico" → también son el caso `/returnai-art`, no genérico). Corregido en bloque en las 22 páginas de `website/` + el selector JS de propagación de UTMs (`a[href*="tidycal.com"]` → `a[href*="agenda.digitalchangeadvisors.com"]`) en las 8 páginas (7 ABM + `/ara`) que lo llevaban, que había quedado apuntando a un patrón que ya no existía en el HTML y dejó de propagar `?utm_*` en silencio.
  - **Confirmado por el usuario (2026-09-09): el proveedor sigue siendo TidyCal** — `agenda.digitalchangeadvisors.com` es su dominio de marca blanca, no un proveedor distinto. `website/privacy-policy.html` (tabla de terceros, fila "TidyCal") **no requiere cambio** — sigue siendo correcta. Los duplicados en `landings/ara/*` y `landings/landing-ara-arquetipo-*.html` (carpeta `landings/`, no desplegada — `dca-landings` sigue "por crear") se dejan deliberadamente en `tidycal.com/pwgdigital/returnai` sin slug corregido — instrucción explícita del usuario: "Solo se ajustan las páginas en producción". Detalle en memoria → `project_agendamiento_enlaces_canonicos.md`.
