...
- El PDF `dossier-fundadores.pdf` se movió junto con la carpeta — no está enlazado desde el HTML (solo vive en el correo de bienvenida de Brevo), así que el rename no rompe nada del lado del sitio; si Brevo ya tenía la URL vieja del PDF configurada en la plantilla, hay que actualizarla ahí manualmente.
